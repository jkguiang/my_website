...
const projects = {
    ...
    "AutoDQM": {
        "title":"AutoDQM",
        "desc_long":"AutoDQM is a statistical tool for Data Quality Monitoring (DQM) at the Large Hadron Particle Collider. Before AutoDQM, DQM shifters were tasked with looking at hundreds of DQM histograms, looking for hard-to-spot issues in data collection. To expedite this process, AutoDQM runs long established statistical tests using a Python backend on these graphs and outputs outliers on a simple React-Javascript frontend that expedites the shifters' task. AutoDQM has and continues to provide modern solutions for DQM, pushing the boundary towards effective and reliable automation.",
    ...
    "ChronoSCAD": {
        "title":"ChronoSCAD",
        "desc_long":"ChronoSCAD renders different prototype configurations of the MIP Timing Detector's Endcap Layer for use in simulating the geometric dependence of the detector. It uses OpenSCAD, a modular, C-like language for rendering 3D objects. ChronoSCAD's output is also useful for various PR purposes, such as cost analysis or pictures and animations for proposals and conceptualization aids. Overall, ChronoSCAD has already proved to be invaluable for answering important proposal questions and its dynamic design ensures its future utility.",
